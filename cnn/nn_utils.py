...
def draw_conv_filters_torch(epoch, step, weight, save_dir):
    w = weight.data.detach().numpy()
    N, C, H, W = w.shape
    w = w.reshape(N, H * W)
    # print(w.shape)
    num_filters = w.shape[0]
...
def draw_conv_filters_torch_multichannel(epoch, step, weights, save_dir):
    w = weights.data.detach().numpy()
    w = w.copy()
    num_filters = w.shape[0]
