...
class FC(Layer):
  ...
  def forward(self, inputs):
    ...
    assert output.shape == (self.N, self.num_outputs)
    return output
  ...
  def backward_inputs(self, grads):
    ...
    """
    output = np.dot(grads, self.weights)
    assert output.shape == (self.N, self.num_inputs)
    return output
...
class ReLU(Layer):
  ...
  def backward_inputs(self, grads):
    ...
    Returns:
      ndarray of shape (N, C, H, W).
    """
    return np.where(self.inputs > 0, grads, 0)
...
class SoftmaxCrossEntropyWithLogits():
  ...
  def forward(self, x, y):
    ...
    """
    l = np.log(np.sum(np.exp(x), axis=1)) - np.sum(x*y, axis=1)
    return np.average(l)
  ...
  def backward_inputs(self, x, y):
    ...
    output = 1/n * (numerator/denominator - y)
...
class L2Regularizer():
  ...
  def forward(self):
    ...
    """
    return np.sum(np.linalg.norm(self.weights)**2*self.weight_decay)
  ...
  def backward_params(self):
    ...
    """
    grad_weights = 2*self.weights*self.weight_decay
