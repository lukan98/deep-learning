...
class RecurrentNetwork(nn.Module):
    def __init__(self,
                 ...
                 num_layers: int = 2):
        super().__init__()
        self.recurrent_layers = nn.ModuleList([nn.RNN(input_size=300, hidden_size=150, num_layers=num_layers),
                                               nn.RNN(input_size=150, hidden_size=150, num_layers=num_layers)])
        self.layers = nn.ModuleList([nn.Linear(in_features=150, out_features=150),
                                     nn.ReLU(),
...
if __name__ == '__main__':
    ...
    model = RecurrentNetwork(train_dataset, freeze=True)
