...
class BaselineModel(nn.Module):
    def __init__(self, dataset: data_loading.NLPDataset, freeze: bool):
        super().__init__()
        self.layers = nn.ModuleList([nn.Linear(in_features=300, out_features=150),
                                     nn.ReLU(),
...
def train(model: BaselineModel, data: DataLoader,
...
def evaluate(model: BaselineModel, data: DataLoader, criterion):
    model.eval()
    with torch.no_grad():
        y_hat = None
        y = None
...
if __name__ == '__main__':
    ...
    model = BaselineModel(train_dataset, freeze=False)
