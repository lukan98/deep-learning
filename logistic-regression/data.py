...
def sample_validation_set(X, Y, sample_size):
    if (not torch.is_tensor(X) or not torch.is_tensor(Y)):
        raise ValueError("X and Y must both be torch.Tensor data types!")
    if (not len(X) == len(Y)):
        raise ValueError("X and Y must be the same size!")

    sampled_indices = torch.tensor(random.sample(range(len(X)), sample_size))
    X_val = X[sampled_indices]
    Y_val = Y[sampled_indices]
    X_train = np.delete(X, sampled_indices, axis=0)
    Y_train = np.delete(Y, sampled_indices, axis=0)
